            ...
            try:
                ...
                for m in members:
                    if m['username'] not in existing_usernames:
                        cursor.execute('''INSERT INTO members (username, rank, gender, member_since, posts, reactions_received, points) 
                                          VALUES (?, ?, ?, ?, ?, ?, ?)''', 
                                       (m['username'], m['rank'], m['gender'], m['memberSince'], m['posts'], m['reactions'], m['points']))
                        new_found += 1
                        existing_usernames.add(m['username'])
                
                conn.commit()
                print(f"Found {new_found} new members on page {page_no}.")
...
if __name__ == "__main__":
    asyncio.run(scrape_new_members())
